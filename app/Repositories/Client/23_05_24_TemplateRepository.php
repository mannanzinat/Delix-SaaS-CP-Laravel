...
class TemplateRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if (is_array($request->type_of_action) && isset($request->type_of_action)) {
                $buttons = [];
                foreach ($request->type_of_action as $key => $action) {
                    $button_text = $request->button_text[$key];
                    $button_value = $request->button_value[$key];
    ...
    {
        return $this->model->withPermission()->where('type', TypeEnum::WHATSAPP)->latest()->paginate();
    }
    public function activeWhatsappTemplate()
    {
        return $this->model->withPermission()->where('type', TypeEnum::WHATSAPP)->latest()->get();
    }
}
