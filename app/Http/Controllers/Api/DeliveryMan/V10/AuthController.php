...
class AuthController extends Controller
{
    ...
    public function loginOtp(Request $request)
    {
        ...
        try {
            ...
            if($this->passwordReset($user, $password)):
                ...
                elseif($this->send($sms_body, $user->phone_number, 'login-otp', setting('active_sms_provider'))):
                    DB::commit();
                    return $this->responseWithSuccess(__('an_otp_send_to_your_phone_number'),'', [] ,200);
                else:
    ...
    public function login(Request $request)
    {
        try {
            ...
            $total_pickup_delivery_commission_income = DeliveryManAccount::whereIn('source', ['pickup_commission', 'parcel_delivery'])
                                                                        ->where('type', 'income')
                                                                        ->where('delivery_man_id', $user->deliveryMan->id)
                                                                        ->sum('amount');
            $total_pickup_delivery_commission_expense = DeliveryManAccount::whereIn('source', ['pickup_commission', 'parcel_delivery'])
                                                                        ->where('type', 'expense')
                                                                        ->where('delivery_man_id', $user->deliveryMan->id)
                                                                        ->sum('amount');

            $total_earning = number_format($total_pickup_delivery_commission_income - $total_pickup_delivery_commission_expense, 2) . ' ' . __('tk');
            ...
            $pending_amount = number_format($total_cash_collect_income - $total_cash_collect_expense, 2) . ' ' . __('tk');
            $commission_received = $total_pickup_delivery_commission_income;
    ...
    public function statistics(Request $request)
    {
        try {
            ...
            $total_pickup_delivery_commission_income = DeliveryManAccount::whereDate('created_at', '>=', $filter_start_date)
                                                        ->whereDate('created_at', '<=', $filter_end_date)->whereIn('source', ['pickup_commission', 'parcel_delivery'])
                                                        ->where('type', 'income')
                                                        ->where('delivery_man_id', $user->deliveryMan->id)
                                                        ->sum('amount');
            ...
            $data = [
                ...
                'financial_statistics' => [
                    'cash_collection'       => $total_cash_collect_income,
            ...
            return $this->responseWithSuccess(__('Statistics retrived successfully'), '', $data, 200);
        } catch (\Exception $e) {
            return $this->responseWithError(__('something_went_wrong_please_try_again'), [], 500);
        }
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        try{
            ...
            $delivery_man->driving_license        = $request->file('driving_license') ? $this->imageUpload($request->file('driving_license'), 'driving-license', $request->id) : '';
    ...
    public function getProfile($user)
    {
        ...
        if (isset($user->image)):
            $data['image'] = optional($user->image)->image_small_two;
        else:
            $data['image'] = getFileLink('80X80', []);
        endif;
        $data['address'] = $user->deliveryMan['address'];
        $data['driving_license'] = $user->deliveryMan['driving_license'] ?? '';
    ...
    public function forgotPasswordOtp(Request $request)
    {
        ...
        try {
            ...
            if($this->passwordResetOtp($user, $otp)):
                ...
                if($this->test($sms_body, $user->phone_number, 'reset-password', setting('active_sms_provider'))):
                    DB::commit();
