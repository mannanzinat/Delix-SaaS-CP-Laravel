                ...
                <ul class="link-list-opt no-bdr">
                    ...
                    @if (hasPermission('withdraw_process'))
                        @if ($payment->status == 'pending' || $payment->status == 'approved')
                            @if ($payment->status == 'pending')
                                <li>
                                    @if (hasPermission('add_to_bulk_withdraw'))
                                        <a href="javascript:void(0);" class="approve-withdraw dropdown-item"
                                            id="approve-withdraw" data-url="{{ route('get-batches') }}"
                                    ...
                                    @else
                                        <a href="javascript:void(0);"
                                            onclick="changeWithdrawStatus('{{ '/admin/withdraw-status/' }}', {{ $payment->id }}, 'approved')" data-id="{{ $payment->id }}"
                                            class="dropdown-item">
                                            <span>{{ __('approve') }}</span>
                                        </a>
                                    @endif
                            ...
                            @if ($payment->status == 'approved')
                                <li>
                                    <a href="javascript:void(0);" class="change-batch dropdown-item" id="change-batch" data-id="{{ $payment->id }}"
                                        data-bs-toggle="modal" data-bs-target="#batch-change"
                            ...
                            <li>
                                <a href="javascript:void(0);" class="process-withdraw dropdown-item"
                                    id="process-withdraw" data-bs-toggle="modal" data-bs-target="#withdraw-process"
                                    data-id="{{ $payment->id }}">
                    ...
                    @if (hasPermission('withdraw_reject'))
                        @if (
                            ...
                            <li>
                                <a href="javascript:void(0);" class="reject-payment dropdown-item" id="reject-payment" data-id="{{ $payment->id }}"
                                    data-bs-toggle="modal" data-bs-target="#payment-reject">
                                    <span> {{ __('reject') }} </span>
                                </a>
                            </li>
                        @endif
