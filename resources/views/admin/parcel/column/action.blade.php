            ...
            <ul class="dropdown-menu">
                @if (hasPermission('parcel_update'))
                    @if (
                        ...
                        <li>
                            <a class="dropdown-item" href="{{ route('parcel.edit', $parcel->id) }}"
                                data-id="{{ $parcel->id }}">
                                <span> {{ __('edit') }}</span>
                            </a>
                        </li>
                ...
                @if ($parcel->status != 'pending' && $parcel->status != 'delivered-and-verified' && hasPermission('parcel_backward'))
                    @if ($parcel->status == 'cancel' || $parcel->status == 'deleted')
                        <li><a href="javascript:void(0);" class="reverse-from-cancel dropdown-item"
                                data-id="{{ $parcel->id }}" id="reverse-from-cancel" data-bs-toggle="modal"
                                data-bs-target="#parcel-reverse-from-cancel"><span>
                                    {{ __('backward') . ' (' . __($parcel->status_before_cancel) }}) </span></a></li>
                    @else
                        <li><a href="javascript:void(0);" class="delivery-reverse dropdown-item"
                                data-id="{{ $parcel->id }}" id="reverse-delivery" data-bs-toggle="modal"
                                data-bs-target="#delivery-reverse"
                ...
                @if (
                    ...
                    @if (hasPermission('parcel_returned_to_warehouse'))
                        <li><a href="javascript:void(0);" class="delivery-return dropdown-item"
                                data-id="{{ $parcel->id }}" id="delivery-return" data-bs-toggle="modal"
                                data-bs-target="#return-delivery"><span>
                ...
                @if ($parcel->status == 'pending')
                    @if (hasPermission('parcel_pickup_assigned'))
                        <li><a href="javascript:void(0);" class="assign-pickup-man dropdown-item"
                                data-id="{{ $parcel->id }}" id="assign-pickup-man" data-bs-toggle="modal"
                                data-bs-target="#assign-pickup"> <span>
                                    {{ __('assign_pickup_man') }} </span></a></li>
                    @endif

                    @if (hasPermission('parcel_delete'))
                        <li><a href="javascript:void(0);" class="delete-parcel dropdown-item"
                                data-id="{{ $parcel->id }}" id="delete-parcel" data-bs-toggle="modal"
                                data-bs-target="#parcel-delete"> <span> {{ __('delete') }} </span></a></li>
                    @endif
                @elseif($parcel->status == 'pickup-assigned' || $parcel->status == 're-schedule-pickup')
                    @if (hasPermission('parcel_reschedule_pickup'))
                        <li><a href="javascript:void(0);" class="reschedule-pickup dropdown-item"
                                data-id="{{ $parcel->id }}" id="reschedule-pickup" data-bs-toggle="modal"
                                data-bs-target="#re-schedule-pickup"> <span>
                    ...
                    @if (hasPermission('parcel_received'))
                        <li>
                            <a href="javascript:void(0);" class="receive-parcel dropdown-item"
                                data-id="{{ $parcel->id }}" id="receive-parcel" data-bs-toggle="modal"
                                data-bs-target="#parcel-receive">
                    ...
                    @if (hasPermission('parcel_delete'))
                        <li>
                            <a href="javascript:void(0);" class="delete-parcel dropdown-item"
                                data-id="{{ $parcel->id }}" id="delete-parcel" data-bs-toggle="modal"
                                data-bs-target="#parcel-delete">
                                <span> {{ __('delete') }}</span>
                            </a>
                        </li>
                    @endif
                @elseif($parcel->status == 'received-by-pickup-man')
                    ...
                    @if (hasPermission('parcel_cancel'))
                        <li>
                            <a href="javascript:void(0);" class="cancel-parcel dropdown-item"
                                data-id="{{ $parcel->id }}" id="cancel-parcel" data-bs-toggle="modal"
                                data-bs-target="#parcel-cancel">
                                <span> {{ __('cancel') }} </span>
                            </a>
                        </li>
                    @endif
                @elseif($parcel->status == 'received' || $parcel->status == 'transferred-received-by-branch')
                    @if (hasPermission('parcel_transfer_to_branch'))
                        <li>
                            <a href="javascript:void(0);" class="transfer-to-branch dropdown-item"
                                data-id="{{ $parcel->id }}" id="transfer-to-branch"
                                data-url="{{ route('parcel.transfer.options') }}" data-bs-toggle="modal"
                    ...
                    @if (hasPermission('parcel_delivery_assigned'))
                        <li>
                            <a href="javascript:void(0);" class="assign-delivery-man dropdown-item"
                                data-id="{{ $parcel->id }}" id="assign-delivery-man" data-bs-toggle="modal"
                                data-bs-target="#assign-delivery">
                ...
                @elseif($parcel->status == 'delivery-assigned' || $parcel->status == 're-schedule-delivery')
                    @if (hasPermission('parcel_reschedule_delivery'))
                        <li>
                            <a href="javascript:void(0);" class="reschedule-delivery dropdown-item"
                                data-id="{{ $parcel->id }}" id="reschedule-delivery" data-bs-toggle="modal"
                                data-bs-target="#re-schedule-delivery">
                    ...
                    @if (hasPermission('parcel_returned_to_warehouse'))
                        <li>
                            <a href="javascript:void(0);" class="delivery-return dropdown-item"
                                data-id="{{ $parcel->id }}" id="delivery-return" data-bs-toggle="modal"
                                data-bs-target="#return-delivery"> <span>
                    ...
                    @if (hasPermission('parcel_delivered'))
                        ...
                        <li>
                            <a href="javascript:void(0);" class="delivery-parcel dropdown-item"
                                data-id="{{ $parcel->id }}" id="delivery-parcel" data-bs-toggle="modal"
                                data-bs-target="#parcel-delivered"> <span>
                                    {{ __('delivered') }} </span>
                            </a>
                        </li>
                    @endif
                    ...
                    @if (hasPermission('parcel_returned_to_merchant'))
                        <li>
                            <a href="javascript:void(0);" class="parcel-returned-to-merchant dropdown-item"
                                data-id="{{ $parcel->id }}" id="parcel-returned-to-merchant"
                                data-bs-toggle="modal" data-bs-target="#returned-to-merchant">
