...
<div class="text-center">
    ...
    @elseif($parcel->status == 'pickup-assigned')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('pickup_assigned') }}</span><br>
    ...
    @elseif($parcel->status == 'received-by-pickup-man')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('received_by_pickup_man') }}</span><br>
    ...
    @elseif($parcel->status == 'transferred-to-branch')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('transferred_to_branch') }}</span><br>
    @elseif($parcel->status == 'transferred-received-by-branch')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('transferred_received_by_branch') }}</span><br>
    @elseif($parcel->status == 'delivery-assigned')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('delivery_assigned') }}</span><br>
    @elseif($parcel->status == 're-schedule-delivery')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('re_schedule_delivery') }}</span><br>
    @elseif($parcel->status == 'returned-to-warehouse')
        @if($parcel->is_partially_delivered)
            <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('partially_delivered') }}</span><br>
        @endif
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('returned_to_warehouse') }}</span><br>
    @elseif($parcel->status == 'return-assigned-to-merchant')
        @if($parcel->is_partially_delivered)
            <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('partially_delivered') }}</span><br>
        @endif
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('return_assigned_to_merchant') }}</span><br>
    ...
    @elseif($parcel->status == 'delivered-and-verified')
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('delivered_and_verified') }}</span><br>
    @elseif($parcel->status == 'returned-to-merchant')
        @if($parcel->is_partially_delivered)
            <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('partially_delivered') }}</span><br>
        @endif
        <span  class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{ __('returned_to_merchant') }}</span><br>
    ...
    @if($parcel->location != 'dhaka' && ($parcel->status == 'delivery-assigned' || $parcel->status == 're-schedule-delivery') && $parcel->third_party_id != '')
        <span class="badge {{ config('parcel_status.' . $parcel->status . '.color') }}">{{__('third_party')}}</span><br><br>
    @endif
</div>
