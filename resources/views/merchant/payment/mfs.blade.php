                                        ...
                                        @foreach($methods as $method)
                                            ...
                                            <div class="row g-gs">
                                                <div class="col-md-6 mb-2">
                                                    <label class="form-label" for="mfs_number">{{ $method->name . ' ' . __('number') }}</label>
                                                    <input type="text" class="form-control" id="mfs_number" value="{{ old('mfs_number.' . $loop->index, @$method->payment->mfs_number) }}" name="mfs_number[]">
                                                ...
                                                <div class="col-md-6 mb-2">
                                                    <label class="form-label" for="mfs_ac_type">{{ $method->name . ' ' . __('account_type') }} <span class="text-danger">*</span></label>
                                                    <select class="without_search form-select form-control" name="mfs_ac_type[]">
                                                        <option value="">{{ __('select_type') }}</option>
                                                        @foreach (\Config::get('parcel.account_types') as $type)
                                                            <option value="{{ $type }}" {{ old('mfs_ac_type.' . $loop->index, @$method->payment->mfs_ac_type) == $type ? 'selected' : '' }}>
                                                                {{ __($type) }}
                                                            </option>
                                                        @endforeach
                                                    </select>
